...
//AA 0F A1 01 12 01 AB 00 00 02 34 09 45 00 00 6D 55  //错误，第二个Y坐标超范围
			...
			for(uint8_t i = 2;i<=14;i++) //计算校验值
      {
			...
			USART1_SendStatus(STATUS_COMMAND_REJECTED);
			Beep(200); //蜂鸣器报警，数据校验失败，原因是校验值错误或参数超出范围
			return(0);
...
void USART1_SendStatus(uint8_t Status)
{
	...
	StatusBuf[1] = 0x03;
	StatusBuf[2] = Status;
	StatusBuf[3] = StatusBuf[1] ^ StatusBuf[2];
	StatusBuf[4] = 0x55;
	HAL_UART_Transmit(&huart1, StatusBuf, 5, 10);
}
