...
//	uint8_t MenuName = MENU_MAIN_WalkAside;
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_TIM_OC_Start_IT(&htim4,TIM_CHANNEL_1);//保持原来的1ms按键、ADC和蜂鸣器计时
	SERVO_Init();//启动PB8舵机，并转到90度初始位置
	Beep(100);
	HAL_Delay(100);
	OLED_Init();
...
//SERVO_MoveToAngle(45);
//HAL_Delay(2000);

//SERVO_MoveToAngle(90);
//HAL_Delay(2000);

//SERVO_MoveToAngle(135);
//HAL_Delay(2000);

//SERVO_MoveToAngle(180);
//HAL_Delay(2000);

//SERVO_MoveToAngle(270);
//HAL_Delay(2000);
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	{
		NextCompare = __HAL_TIM_GET_COMPARE(&htim4, TIM_CHANNEL_1) + 1000U;
		if(NextCompare >= SERVO_PWM_PERIOD)
		{
			NextCompare -= SERVO_PWM_PERIOD;
		}
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, NextCompare);
		...
		HAL_ADC_Start_IT(&hadc1);//保持原来的1ms ADC按键采样
	}
}
